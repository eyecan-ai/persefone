...
class RandomStain(ImageOnlyTransform):
    ...
    :param min_rgb: minimum rgb value
    :type min_rgb: Tuple[int, int, int]
    :param max_rgb: maximum rgb value
    :type max_rgb: Tuple[int, int, int]
    :param n_points: number of points per stain
    :type n_points: int
...
if __name__ == '__main__':
    ...
    imgs = []
    stain = RandomStain(3, 8, 16, 64, 1, 2, (0, 0, 0), (255, 255, 255), 20, 5, True, 1)
    for i in range(16):
        img = stain(image=np.zeros((256, 256, 3)) + np.random.randint(0, 256, (3,)))['image']
